...
<p align="center">
  English |
  <a href="https://github.com/Vizards/deepseek-v4-for-copilot/blob/main/README.zh-cn.md">ÁÆÄ‰Ωì‰∏≠Êñ?/a>
</p>

**Pick DeepSeek V4 from the Copilot Chat model picker ‚Ä?and keep everything else Copilot already gives you.**
...
Love DeepSeek's price-performance but don't want to give up GitHub Copilot's agent mode, tool calling, and polished UI? This extension drops **DeepSeek V4 Pro & Flash** straight into the Copilot Chat model selector ‚Ä?with **vision**, **thinking mode**, and your own API key.

## Why this extension?

- **Don't replace Copilot ‚Ä?power it up.** No new sidebar, no new chat UI to learn. Just a new model in the picker you already use.
- **Agent mode, tool calling, instructions, MCP, skills ‚Ä?all of it still works.** Copilot's entire stack, now running on DeepSeek.
- **Vision on a text-only model.** DeepSeek V4 can't see images. This extension proxies any image you drop into chat through another Copilot model you already have, then feeds the description to DeepSeek ‚Ä?transparently.
...
DeepSeek V4 is text-only. Drop a screenshot into chat and this extension automatically hands the image to another installed Copilot model (Claude, GPT-4o, whatever you've got), gets a description, and feeds that back to DeepSeek. **Zero config** ‚Ä?just pick your preferred vision model once.
...
- **Agent mode** ‚Ä?autonomous multi-step tasks
- **Tool calling** ‚Ä?file edits, terminal, workspace search, Git, tests
- **Instructions & skills** ‚Ä?all your `.instructions.md`, `AGENTS.md`, and skills just work
- **Prompt caching stats** ‚Ä?DeepSeek's cache hit rate logged in the output channel so you can see the savings
...
- VS Code 1.116 or later. This extension relies on non-public Copilot Chat APIs that may break on newer VS Code versions ‚Ä?[report an issue](https://github.com/Vizards/deepseek-v4-for-copilot/issues) if you hit one.
- GitHub Copilot subscription (Free / Pro / Enterprise ‚Ä?the free tier works)
...
1. **Microsoft VS Code** ‚Ä?install from [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=Vizards.deepseek-v4-for-copilot).
2. **Editors that use Open VSX** ‚Ä?install from [Open VSX](https://open-vsx.org/extension/Vizards/deepseek-v4-for-copilot).
...
|---|---|---|
| `deepseek-copilot.baseUrl` | `https://api.deepseek.com` | API endpoint ‚Ä?change for self-hosted / proxied deployments |
...
|---|---|---|---|
| Works inside Copilot Chat | ‚ú?| ‚ú?| ‚ù?separate UI |
| Agent mode, tools, skills | ‚ú?| ‚ú?| ‚ö†Ô∏è reimplemented |
| Vision support | ‚ú?proxied | ‚ù?| ‚ù?|
| No extra process to run | ‚ú?| ‚ù?| ‚ú?|
| One-click install | ‚ú?| ‚ù?| ‚ú?|
| API key in OS keychain | ‚ú?| ‚ù?| ‚ö†Ô∏è varies |
